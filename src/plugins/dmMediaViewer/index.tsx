/*
 * DM Media Viewer - Vencord UserPlugin
 * Place this file at: src/userplugins/dmMediaViewer([docs.vencord.dev](https://docs.vencord.dev/plugins/)) useMemo, useState } from "react";
import { Forms, Margins, ModalRoot, ModalContent, ModalHeader, ModalCloseButton, openModal } from "@webpack/common";
...
function inferType(url: string): MediaType | null {
    ...
    if (lowered.includes("/attachments/") && lowered.match(/\.(png|jpg|jpeg|gif|webp|bmp|avif)(?:$|\?)/)) return "image";
    if (lowered.includes("/attachments/") && lowered.match(/\.(mp4|webm|mov|mkv|m4v)(?:$|\?)/)) return "video";
...
function extractMediaFromDom(): MediaItem[] {
    ...
    const messageNodes = Array.from(document.querySelectorAll("[id^='chat-messages-'], [class*='messageListItem'], [class*='message_"]"));
    ...
    for (const node of messageNodes) {
        ...
        for (const video of vids) {
            const src = (video as HTMLVideoElement).src || video.querySelector("source")?.src;
            if (!src) continue;
            if (!src.includes("cdn.discordapp.com") && !src.includes("media.discordapp.net")) continue;
...
function isProbablyDm(): boolean {
    ...
    return /\/channels\/@me\//.test(path);
}

function GalleryModal(props: { onClose: () => void; items: MediaItem[]; channelName: string; }) {
    ...
    }, [props.items, query, tab]);

    return (
        <ModalRoot size="large">
            <ModalHeader separator={false} className={Margins.bottom16}>
                <Forms.FormTitle tag="h2">DM Media Viewer — {props.channelName}</Forms.FormTitle>
                <ModalCloseButton onClick={props.onClose} />
            </ModalHeader>

            <ModalContent>
                <div style={{ display: "flex", gap: 8, marginBottom: 12, alignItems: "center", flexWrap: "wrap" }}>
                    <button onClick={() => setTab("all")}>All ({props.items.length})</button>
                    <button onClick={() => setTab("image")}>Images ({props.items.filter(x => x.type === "image").length})</button>
                    <button onClick={() => setTab("video")}>Videos ({props.items.filter(x => x.type === "video").length})</button>
                    <input
                        value={query}
...
function injectButton() {
    const existing = document.getElementById("vc-dm-media-viewer-button");
    if (existing) return;
